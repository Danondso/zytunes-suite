...
- **iPod device-panel identity** — generation + capacity composed from SysInfo `ModelNumStr` (libgpod suffix table: 1G–4G, Photo, mini, shuffle, nano 1–6, Video, Classic), SysInfoExtended `FamilyID`, USB product ID, and snapped storage. Unknown SKUs never guess Classic. Panel title uses the iTunes name from `iPod_Control/iTunes/DeviceInfo` (UTF-16LE) when present; otherwise the model string. A Model line is shown only when it differs from the title. USB mode reports Mass Storage.
