...
- **Audio playback** — in-TUI preview of library tracks via rodio. `Space` play/pause, `n`/`p` next/previous, `<` / `>` (or `,` / `.`) scrub ±5 seconds. Press `P` to cycle the now-playing panel through auto → force-hidden → force-shown. The now-playing panel draws a live spectrum soundbar from the playing mix (stem mutes included). Each theme has its own layout, glyph alphabet (blocks, braille, ASCII, …), and colouring; `W` overrides the layout until you change theme. The bar is centered, only changes height, and shows a Hz shorthand under each band. Selected rows marquee-scroll long titles
