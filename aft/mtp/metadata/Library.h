...
namespace mtp
{
	...
	private:
		...
		Library(const SessionPtr &session, StorageId storage);
	...
	public:
		...
		bool SaveCache(const std::string &path, const std::string &deviceSerial) const;
		static std::shared_ptr<Library> TryLoadFromCache(const std::string &path, const SessionPtr &session, const std::string &deviceSerial);

		void SetCachePath(const std::string &path, const std::string &deviceSerial)
		{ _cachePath = path; _cacheSerial = deviceSerial; }
		void AutoSave() const;
