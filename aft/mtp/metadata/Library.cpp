...
namespace mtp
{
	...
	Library::Library(const SessionPtr &session, StorageId storage):
		_session(session),
		_storage(storage),
		_artistSupported(false),
		_albumDateAuthoredSupported(false),
		_albumCoverSupported(false)
	{}

	bool Library::SaveCache(const std::string &path, const std::string &deviceSerial) const
	{
		...
		f << "AFT-LIBRARY-CACHE-V1\n";
		f << deviceSerial << "\n";
		f << _storage.Id << "\n";
		f << (_artistSupported ? 1 : 0) << " "
		  << (_albumDateAuthoredSupported ? 1 : 0) << " "
		  << (_albumCoverSupported ? 1 : 0) << "\n";
		f << _artistsFolder.Id << " " << _albumsFolder.Id << " " << _musicFolder.Id << "\n";

		// Write artists: one per line, tab-separated: id \t musicFolderId \t name
		f << _artists.size() << "\n";
		for (const auto &pair : _artists)
		{
			const auto &artist = pair.second;
			f << artist->Id.Id << "\t" << artist->MusicFolderId.Id << "\t" << artist->Name << "\n";
		}

		// Write albums: one per line, tab-separated: id \t musicFolderId \t year \t artistName \t albumName
		f << _albums.size() << "\n";
		for (const auto &pair : _albums)
		{
			const auto &album = pair.second;
			f << album->Id.Id << "\t" << album->MusicFolderId.Id << "\t"
			  << static_cast<long long>(album->Year) << "\t"
	...
	std::shared_ptr<Library> Library::TryLoadFromCache(const std::string &path, const SessionPtr &session, const std::string &deviceSerial)
	{
		std::ifstream f(path);
		if (!f)
			return nullptr;
		...
		// Check header
		if (!std::getline(f, line) || line != "AFT-LIBRARY-CACHE-V1")
			return nullptr;

		// Check device serial
		if (!std::getline(f, line) || line != deviceSerial)
		{
			debug("cache serial mismatch, ignoring cache");
			return nullptr;
		}
		...
		auto lib = LibraryPtr(new Library(session, StorageId(storageId)));

		int as, ads, acs;
		if (!(f >> as >> ads >> acs))
			return nullptr;
		lib->_artistSupported = as;
		lib->_albumDateAuthoredSupported = ads;
		lib->_albumCoverSupported = acs;

		u32 af, alf, mf;
		if (!(f >> af >> alf >> mf))
			return nullptr;
		lib->_artistsFolder = ObjectId(af);
		lib->_albumsFolder = ObjectId(alf);
		lib->_musicFolder = ObjectId(mf);

		// Consume the newline after the last >> read
		std::getline(f, line);
		...
		for (size_t i = 0; i < artistCount; ++i)
		{
			...
			u32 id, musicFolderId;
			if (!(iss >> id))
				return nullptr;
			char tab;
			iss.get(tab); // consume tab
			if (!(iss >> musicFolderId))
				return nullptr;
			iss.get(tab); // consume tab
			...
			auto artist = std::make_shared<Artist>();
			artist->Id = ObjectId(id);
			artist->MusicFolderId = ObjectId(musicFolderId);
			artist->Name = name;
			lib->_artists.insert(std::make_pair(name, artist));
		}
		...
		for (size_t i = 0; i < albumCount; ++i)
		{
			...
			// Read artist name (tab-separated from album name)
			std::string artistName, albumName;
			if (!std::getline(iss, artistName, '\t'))
				return nullptr;
			if (!std::getline(iss, albumName))
				return nullptr;

			auto artistIt = lib->_artists.find(artistName);
			ArtistPtr artist;
			if (artistIt != lib->_artists.end())
				artist = artistIt->second;
			else
			{
				// Artist not in cache (shouldn't happen, but handle gracefully)
				artist = std::make_shared<Artist>();
				artist->Name = artistName;
				lib->_artists.insert(std::make_pair(artistName, artist));
			}

			auto album = std::make_shared<Album>();
			album->Id = ObjectId(id);
			album->MusicFolderId = ObjectId(musicFolderId);
			album->Artist = artist;
			album->Name = albumName;
			album->Year = static_cast<time_t>(year);
			lib->_albums.insert(std::make_pair(std::make_pair(artist, albumName), album));
		}

		debug("loaded library cache: ", lib->_artists.size(), " artists, ", lib->_albums.size(), " albums");
		return lib;
	}

	void Library::AutoSave() const
	{
		if (!_cachePath.empty())
			SaveCache(_cachePath, _cacheSerial);
	}
