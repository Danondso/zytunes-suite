...
namespace cli
{
		...
		_batterySupported(false)
	{
		...
		{
			AddCommand("zune-init", "load media library (uses cache if available)",
			...
			AddCommand("zune-refresh", "clear cache and reload media library from device",
				make_function([this]() -> void { ZuneRefresh(); }));
			AddCommand("create-playlist", "<name> <comma-separated-ids> create a playlist",
				make_function([this](const std::string &name, const std::string &ids) -> void { CreatePlaylist(name, ids); }));
		}
		...
		AddCommand("props-id", "<object-id> show properties for object by ID",
			make_function([this](mtp::u32 id) -> void { ListProperties(mtp::ObjectId(id)); }));
		AddCommand("refs-id", "<object-id> show object references by ID",
			make_function([this](mtp::u32 id) -> void {
				auto refs = _session->GetObjectReferences(mtp::ObjectId(id));
				for (auto ref : refs.ObjectHandles)
					mtp::print(ref.Id);
			}));
...
#undef CASE
	...
	std::string Session::GetLibraryCachePath()
	{
		char *home = getenv("HOME");
		return std::string(home ? home : ".") + "/.aft-library-cache";
	}
	...
	void Session::ZuneInit()
	{
		...
		auto cachePath = GetLibraryCachePath();
		auto serial = _gdi.SerialNumber;

		_library = mtp::Library::TryLoadFromCache(cachePath, _session, serial);
		if (_library)
		{
			mtp::print("loaded library from cache");
			_library->SetCachePath(cachePath, serial);
			return;
		}

		mtp::print("loading library from device (this may take several minutes)...");
		_library = std::make_shared<mtp::Library>(_session);
		_library->SetCachePath(cachePath, serial);
		_library->SaveCache(cachePath, serial);
		mtp::print("library cached to ", cachePath);
	}

	void Session::ZuneRefresh()
	{
		auto cachePath = GetLibraryCachePath();
		std::remove(cachePath.c_str());
		_library.reset();
		mtp::print("cache cleared, reloading library from device...");
		_library = std::make_shared<mtp::Library>(_session);
		auto serial = _gdi.SerialNumber;
		_library->SetCachePath(cachePath, serial);
		_library->SaveCache(cachePath, serial);
		mtp::print("library reloaded and cached");
	}
	...
	void Session::ZuneImport(const LocalPath & path)
	{
		...
		print("track-id:", songId.Id);
	}

	void Session::CreatePlaylist(const std::string & name, const std::string & ids)
	{
		using namespace mtp;

		if (name.empty())
			throw std::runtime_error("playlist name required");

		// Parse comma-separated IDs.
		std::vector<ObjectId> trackIds;
		...
		while (std::getline(ss, token, ','))
		{
			...
			try {
				u32 id = std::stoul(token);
				trackIds.push_back(ObjectId(id));
			} catch (...) {
				throw std::runtime_error("invalid track ID: " + token);
			}
		}

		if (trackIds.empty())
			throw std::runtime_error("at least one track ID required");

		// Create abstract playlist object.
		msg::ObjectInfo oi;
		oi.Filename = name;
		oi.ObjectFormat = ObjectFormat::AbstractAVPlaylist;
		oi.AssociationType = static_cast<mtp::AssociationType>(0);

		auto storageId = GetUploadStorageId();
		auto noi = _session->SendObjectInfo(oi, storageId, mtp::Session::Device);
		// Abstract playlists have no data — send empty object.
		_session->SendObject(std::make_shared<ByteArrayObjectInputStream>(ByteArray()));

		// Set the playlist name property (Zune displays this, not the filename).
		_session->SetObjectProperty(noi.ObjectId, ObjectProperty::Name, name);

		// Set the track references on the playlist.
		msg::ObjectHandles refs;
		refs.ObjectHandles = trackIds;
		_session->SetObjectReferences(noi.ObjectId, refs);

		print("created playlist '", name, "' with ", trackIds.size(), " tracks (object id ", noi.ObjectId.Id, ")");
	}
